#!/usr/bin/env bash
# Isolated Ajit + 25 dummy smoke — separate SQLite DB, no production impact.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
BACKEND="$ROOT/backend"
OUT="$ROOT/scripts/ajit25-smoke-screens"
DB="$BACKEND/smoke_ajit25.sqlite3"
API_PORT="${SMOKE_API_PORT:-8012}"
...
.venv/bin/python manage.py seed_ajit25_smoke --json 2>/dev/null | python3 -c "import sys; t=sys.stdin.read(); i=t.find('{'); sys.stdout.write(t[i:] if i>=0 else t)" > "$OUT/verification.json"
...
node "$ROOT/scripts/ajit25_smoke_playwright.mjs" \
...
echo "Dummy IDs KEPT in $DB until you say: solved"
